...
if (!process.env.MONGODB_URI) {
    console.error("FATAL ERROR: MONGODB_URI is not defined in environment variables. Strict singular connection enforced.");
    process.exit(1);
}
...
wss.on('connection', (ws, req) => {
    ...
    if (ws.roomName) {
        const roomId = ws.roomName.replace('room-', '').replace('-shared', '');
        if (roomTerminationTimers.has(roomId)) {
            console.log(`User reconnected to room ${roomId}. Cancelling 3-minute termination timer.`);
            clearTimeout(roomTerminationTimers.get(roomId));
    ...
    ws.on('close', async () => {
        const targetRoom = ws.roomName;
        if (!targetRoom) return;

        // Check if there are any other active connections to the same room
        let activeCount = 0;
        for (const client of wss.clients) {
            if (client.roomName === targetRoom && client.readyState === 1 /* OPEN */) {
                activeCount++;
            }
        }

        console.log(`Client disconnected from ${targetRoom}. Active clients remaining: ${activeCount}`);

        if (activeCount === 0) {
            const roomId = targetRoom.replace('room-', '').replace('-shared', '');
            console.log(`Room ${roomId} has 0 users. Starting 3-minute termination timer.`);
            ...
            const timer = setTimeout(async () => {
                try {
                    ...
                    if (room && room.status !== 'finished') {
                        ...
                        await room.save();
                        console.log(`Room ${roomId} has been automatically terminated after 3 minutes of inactivity.`);

                        global.terminateRoomSockets(roomId);
                    }
                } catch (err) {
                    console.error(`Error automatically terminating room ${roomId}:`, err);
                } finally {
                    roomTerminationTimers.delete(roomId);
                }
            }, 3 * 60 * 1000); // 3 minutes
...
mongoose.connect(mongoURI)
    .then(async () => {
        ...
        server.listen(availablePort, () => {
            console.log(`Server is running on dynamic port ${availablePort}`);
        });
    })
    .catch((err) => {
