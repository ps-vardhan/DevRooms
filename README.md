# DevRooms: Live Coding Education Platform
...
- **Real-Time Collaborative Editing**
  Synchronizes all text edits, cursor positions, and selections across all active users in the room instantly. This creates a highly engaging and interactive coding environment.
...
The backend ([server.js](file:///d:/DevRooms/server/server.js)) exposes a websocket server under `/yjs`. When a workspace room is opened, clients establish a websocket provider connection mapping to `room-${roomId}-shared`.
...
### 2. Collaborative Workspace Synchronization

The frontend [YjsContext.jsx](file:///d:/DevRooms/src/providers/YjsContext.jsx) instantiates:

- `sharedDoc` (bound to the websocket connection).

All updates are synchronized directly through standard Yjs WebSocket provider channels, ensuring seamless state convergence and consistency across all participants in the room.
