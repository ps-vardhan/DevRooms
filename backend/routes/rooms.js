...
roomsRouter.get('/:roomId', userAuth, async (req, res) => {
    try {
        ...
        const room = await Room.findOne({ roomId }).populate('hostId', 'firstName emailId');
...
roomsRouter.post('/:roomId/problem', userAuth, async (req, res) => {
    try {
        const { roomId } = req.params;
        ...
        if (room.hostId.toString() !== req.user._id.toString()) {
            return res.status(403).json({ message: "Only the host can modify the room problem." });
        }
